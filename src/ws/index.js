...
wss.on('connection', (ws, req) => {
  ...
  try {
    ...
    console.log(`[ws] conexión autenticada: ${payload.username} (${payload.role})`);
    ws.send(JSON.stringify({ type: 'auth_ok', user: payload }));
  } catch (err) {
    console.warn('[ws] conexión rechazada:', err.message);
    ws.send(JSON.stringify({ type: 'auth_error', error: 'Token inválido o expirado' }));
    ws.close();
    return;
  }
